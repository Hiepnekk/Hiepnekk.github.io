...
<?php
    ...
    $dataPost = array(
        'ApiKey'    => '141a4f31bc7e7294bb0604b3509f76e6',
        'Pin'       => $pin,
        'Seri'      => $serial,
        'CardType'  => $type,
        'CardValue' => $amount,
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'https://trumnapthe.vn/api/card',
    ...
    curl_close($curl);
    $bentancoder = json_decode($response, true);

        if($bentancoder['Code'] == '1') {
        //Nạp thẻ thành công
        // if(db_execute('insert into the (serial,pin,menhgia,trangthai,request_id) values ('.$serial.','.$pin.','.$amount.',0,'.$request_id.')')){
        echo '<div style="color:green;">✅ Đã nạp 1275 quân huy vào tài khoản <b>✿Bull²⁰²⁰☆
</b></div>';
        }else{
        echo '<div style="color:green;">✅ Đã nạp 1275 quân huy vào tài khoản <b>✿Bull²⁰²⁰☆
</b></div>';
